...
Driven by `ceiling.py` beside it. Building this file directly is for checking
that a frequency places at all:

    python3 ceiling_gateware.py --build --sync-mhz 100 --dqs
...
An earlier test of ours moved 2048 words in one transaction: 17 us at 120 MHz,
over four times tCSM. Its 220.2 MB/s is therefore a rate the part is not
specified to sustain. `BURST_WORDS` here is 128, which is 2.13 us at the slowest
...
`DDRDLLA` locks. `HyperRAMDQSPHY` keeps LOCK internal, so the two status bits
that would carry it read as constant 1 here -- they are placeholders, not
evidence, until the PHY brings them out.
...
# tCSHI, CS# high between transactions, from the datasheet. Held here by the
# caller, which is where it had to live while HyperRAMDQSInterface's RECOVERY
# state carried `# TODO: implement recovery` and fell through to IDLE. Kept
# after that is fixed: this harness is the thing measuring the controller, and a
# measurement that relies on the thing under test to hold its own timing cannot
# distinguish the two.
T_CSHI_NS = 10.0
...
class HyperRAMCeiling(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # DDRDLLA's LOCK, and the end of the PAUSE sequence that copies its delay
        # code into the IOLOGIC. HyperRAMDQSPHY keeps both internal, so they read
        # as constant 1 here and the status bits carrying them are not evidence.
        #
        # That is a real gap, not a tidy-up: the entire read path's delay codes
        # are invalid until the DLL has locked, so a design reporting a clean
        # read with the DLL unlocked has measured nothing, and nothing above the
        # PHY can tell. Worth exposing upstream.
        dll_locked = Const(1)
        dll_ready = Const(1)
        burstdet = Signal()

        if self.dqs:
            # `rwds` and `dq` raw: those are the pads the DQS primitives drive
            # directly. `clk`, `cs` and `reset` stay ordinary output pins.
            bus = platform.request("ram", 0, dir={"rwds": "-", "dq": "-"})
        ...
        # 398 M write cycles and zero words moved.
        writing = Signal()
        last = Signal()
        start = Signal()
...
def main(argv):
    ...
    out = args.build_dir or str(
        here / "build" / f"manual-{tag}-{args.sync_mhz:g}")
