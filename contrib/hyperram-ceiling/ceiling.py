...
Finds the clock the W956A8 actually stops working at, and what failure looks like.
    ...
    ./ceiling.py --build        # bitstreams only, no board
    ./ceiling.py --run          # board only, uses what is built
    ./ceiling.py                # both
...
from ceiling_gateware import (  # noqa: E402
...
# Written to the log as well as the terminal: a sweep is long enough that
# scrollback is not a record.
LOG = BUILD_ROOT / "ceiling.log"
...
CONFIGURE = [sys.executable, "-m", "apollo_fpga.commands.cli", "configure"]
...
def build_one(ck, dqs, sync):
    ...
    script = (
        "import sys\n"
        f"sys.path[:0]={[str(HERE)]!r}\n"
        "from ceiling_gateware import HyperRAMCeiling\n"
...
def run_one(ck, dqs, sync, target_words, readclksel=0b010):
    ...
    configured = subprocess.run(CONFIGURE + [str(bitstream)], cwd=HERE,
